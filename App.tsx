...
const PasswordSchema = Yup.object().shape({
  passwordLenght: Yup.number()
    ...
    .max(4, 'min 4 characters')
    .required('Lenght is Requried'),
});
...
export default function App() {
  ...
  const createPassword = (characterList: string, passwordLength: number) => {
    ...
    for (let i = 0; i < passwordLength; i++) {
      const charactersIndex = Math.round(Math.random() * characterList.length);
      result += characterList.charAt(charactersIndex);
    }
    return result;
  };
